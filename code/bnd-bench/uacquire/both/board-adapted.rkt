...
(define board? hash?)
(unsafe-require/typed/check "board.rkt"
  (#:struct tile ([column : Column] [row : Row])))

(module* admin typed/racket/base
  (require (submod "..") require-typed-check "../base/types.rkt")
  ...
  (provide (all-from-out (submod "..")) tile-column tile-row tile<=?  tile->string ALL-TILES STARTER-TILES# FOUNDING GROWING MERGING SINGLETON IMPOSSIBLE *create-board-with-hotels affordable?  board-tiles  deduplicate/hotel distinct-and-properly-formed found-hotel free-spot?  grow-hotel growing-which make-board merge-hotels merging-which place-tile set-board size-of-hotel what-kind-of-spot))
...
(module* state typed/racket/base
  (require (submod "..") require-typed-check "../base/types.rkt")
  (require/typed/check "board.rkt"
    (tile-column (-> tile Column))
    (tile-row (-> tile Row))
...
(module* strategy typed/racket/base
  (require (submod "..") require-typed-check "../base/types.rkt")
  (require/typed/check "board.rkt"
    (tile-column (-> tile Column))
    (tile-row (-> tile Row))
